...
const BlogDetail = () => {
        ...
        if (collectionId && articleSlug) {
          ...
          try {
            ...
            setBlog({
              ...
              _id:
                response.data.article._id ||
                `${collectionId}-${response.data.article.slug}`,
          ...
          try {
            ...
            if (
              response.data &&
              response.data.code === "API_STRUCTURE_CHANGED"
            ) {
              setError(
                "This URL format is no longer supported. Articles are now accessed through collection IDs and article slugs."
              );
              setLoading(false);
              return;
            }
            ...
            if (
              response.data &&
              response.data.code === "REDIRECT_TO_NEW_FORMAT"
            ) {
              console.log(
                "Found redirect information for this article",
                response.data
              );
              navigate(
                `/blog/collection/${response.data.collectionId}/${response.data.articleSlug}`,
                { replace: true }
              );
              return;
            }
            ...
            setError(
              "This article could not be found. The URL format is outdated."
            );
            setLoading(false);
            return;
          }
  ...
  const handleCopyLink = async () => {
    ...
    try {
      if (blog && collectionId && articleSlug) {
        await api.post(`/blogs/${collectionId}/${articleSlug}/share`, {
          platform: "copyLink",
        });
      } else if (blog?._id) {
        console.warn(
          "Using deprecated share API. Please update to collection/slug pattern."
        );
      }
    } catch (err) {
  ...
  const authorUsername =
    blog.authorUsername || (blog.author && blog.author.username) || "Anonymous";
        ...
        <div className="bg-primary rounded-3xl mb-8 p-8 text-white">
          ...
          {blog.collectionName && (
            <div className="mt-4 bg-white/10 rounded-lg px-4 py-2 inline-block">
              <Link
                to={`/collections/${blog.collectionId}`}
        ...
        <div className="hs-card">
          ...
          <div className="mt-8 border-t border-gray-200 pt-6">
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                className={`p-3 ${
                  copySuccess ? "bg-green-500" : "bg-gray-600"
                } text-white rounded-full hover:bg-gray-700 transition-colors flex items-center`}
          ...
          <div className="flex mt-8 space-x-4">
            ...
            {isAuthor && blog.collectionId && blog.slug && (
              <Link
                to={`/blog/edit/${blog.collectionId}/${blog.slug}`}
