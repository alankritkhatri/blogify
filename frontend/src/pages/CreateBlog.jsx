...
const CreateBlog = () => {
  ...
  // Load the user's blog collections on component mount
  useEffect(() => {
    if (isAuthenticated) {
  ...
  const fetchUserCollections = async () => {
    try {
      ...
      // If there's no selected collection but we have collections, select the first one
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Updated endpoint using the new route structure
      const response = await api.post(`/blogs/${collectionId}`, { 
        title, 
        content
      });
      
      // Navigate to the article using the new URL structure
      if (response.data.article && response.data.article.slug) {
        navigate(`/blog/collection/${collectionId}/${response.data.article.slug}`);
      } else {
        // Fallback to blog collection detail page
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 max-w-lg mx-auto">
            Share your thoughts, ideas, and stories with the world. Great blogs are authentic, valuable, and engaging.
          </p>
        </div>
        ...
        {!isAuthenticated && (
          <div className="bg-yellow-100 text-yellow-800 p-4 rounded-xl mb-6 max-w-3xl mx-auto">
            ...
            <p>You are not currently authenticated. Please log in to create a blog post.</p>
          </div>
        )}
        ...
        <div className="hs-card max-w-3xl mx-auto">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-6">
              <label htmlFor="blogCollection" className="block text-gray-700 font-medium mb-2">
                Select Blog
              </label>
              {collections.length > 0 ? (
                ...
                <div className="p-2 border rounded bg-yellow-50 text-yellow-800">
                  You don't have any blogs yet. <Link to="/create-collection" className="underline">Create a blog</Link> first to add articles.
                </div>
              )}
            </div>
        ...
        <div className="mt-12 bg-gray-50 rounded-3xl p-8 max-w-3xl mx-auto">
          <h2 className="text-xl font-bold mb-4">Tips for writing great blogs</h2>
