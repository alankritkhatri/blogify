...
router.get("/", async (req, res) => {
  try {
    ...
    console.log(
      `Retrieved ${articles.length} articles (page ${page}, limit ${limit})`
    );
...
router.post("/:collectionId", auth, async (req, res) => {
  try {
    ...
    console.log(
      `New article created: ${title} in blog collection: ${blogCollection.name}`
    );
...
router.get("/:id", async (req, res) => {
  try {
    ...
    console.error(
      `Error handling legacy blog route for ID ${req.params.id}:`,
      error
    );
    return res
      .status(500)
      .json({ error: "Server error processing legacy request" });
  }
});
